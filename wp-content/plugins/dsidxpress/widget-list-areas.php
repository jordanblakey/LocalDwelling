	...
	function form($instance) {
		...
		echo <<<HTML
			...
			<div style="float: right">
				<a href="javascript:void(0);" onclick="dsWidgetListAreas.LaunchLookupList('{$ajaxUrl}?action=dsidx_locations', '{$areaOptionsFieldId}_areaType')">See <span class="{$areaOptionsFieldId}_link_title">{$type_normalized}</span> names</a>
			</div>
			...
			<div id="{$advancedId}_advanced" style="display:none">
				...
				<p>
					...
					<span class="description">See all <span class="{$areaOptionsFieldId}_link_title">{$type_normalized}</span> Names <a href="javascript:void(0);" onclick="dsWidgetListAreas.LaunchLookupList('{$ajaxUrl}?action=dsidx_locations', '{$areaOptionsFieldId}_areaType')">here</a></span>
				</p>
