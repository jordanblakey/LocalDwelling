...
class dsSearchAgent_Admin {
	...
	static function Enqueue($hook) {
		...
		if (($hook == 'post.php' && $_GET['action'] == 'edit') || $hook == 'post-new.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'ds-idx-listings-page') {
			wp_enqueue_style('dsidxpress_admin_options_style', DSIDXPRESS_PLUGIN_URL . 'css/admin-options.css', array(), DSIDXPRESS_PLUGIN_VERSION);
		}
	}
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							<input type="hidden" class="linkInputTextArea" id="dsidxpress-RestrictResultsToState" name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[RestrictResultsToState]" value="<?php echo $account_options->RestrictResultsToState; ?>"></input>
	...
	static function DetailsOptions() {
		...
		<div class="wrap metabox-holder">
			...
			<form method="post" action="options.php">
				...
				<h1>Sharing</h1>
				<table class="form-table">
					<?php $fbAppID = isset($account_options->{'FacebookAppID'})? $account_options->{'FacebookAppID'}:''; ?>
					<tr>
						<th>
							<label for="dsidxpress-ShowPanel_ZillowCB">Facebook App ID:</label>
						</th>
						<td>
							<input type="text" id="dsidxpress-FacebookAppID" name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[FacebookAppID]" value="<?php echo $fbAppID; ?>" /><br />
							<span class="description">
							...
							Visit the <a href="https://developers.facebook.com/apps" target="_blank">Facebook Developers Apps Page</a> to create / find your App ID.
							</span>
						</td>
					</tr>
				</table>
				<h1>Maps</h1>
				<table class="form-table">
					<?php $mapsKey = isset($account_options->{'GoogleMapsAPIKey'})? $account_options->{'GoogleMapsAPIKey'}:''; ?>
					<tr>
						<th>
							<label for="dsidxpress-ShowPanel_ZillowCB">Google Maps API Key:</label>
						</th>
						<td>
							<input type="text" id="dsidxpress-GoogleMapsAPIKey" name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[GoogleMapsAPIKey]" value="<?php echo $mapsKey; ?>" /><br />
							<span class="description">
							Required by Google in some cases. <br /><br />
							If maps are not working on your site, visit the <a href="https://developers.google.com/maps/documentation/javascript/get-api-key" target="_blank">Google Maps APIs Documentation</a> to create / find your key.
							</span>
						</td>
					</tr>
				</table>
